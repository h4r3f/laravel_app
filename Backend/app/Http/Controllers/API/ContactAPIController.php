...
class ContactAPIController extends MController {
    ...
    public function store(Request $request) {
        ...
        $input = $request->all();
        $to = DB::table('settings')->first()->email;
        $from = $input['email'];
        $customer = $input['name'];
        Mail::send('mail.contact', array('data' => $input), function ($message) use( $to, $from, $customer) {
            $message->from($from, $customer);
